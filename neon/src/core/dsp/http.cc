...
absl::Status HttpServer::Init(const utils::Config& config, Engine* engine) {
  LOG(INFO) << "Initializing HTTP server";

  engine_ = engine;

  http_host_ = config.Get<std::string>("dsp.http.host");
  http_port_ = config.Get<int>("dsp.http.port");
