...
absl::Status AudioHttpServer::Start(AudioStream* stream,
        ...
        [stream, header_pages, client_offset, headers_sent](
          ...
          if (!headers_sent) {
            if (!header_pages.empty()) {
              sink.write(reinterpret_cast<const char*>(header_pages.data()),
                         header_pages.size());
            }
            headers_sent = true;
          }
          ...
          if (!result.data.empty()) {
            ...
            return sink.write(reinterpret_cast<const char*>(result.data.data()),
                              result.data.size());
          }
